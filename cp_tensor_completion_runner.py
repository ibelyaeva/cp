...
class CPTensorCompletionRunner(object):
    ...
    def __init__(self, ground_truth_img, ground_truth, tensor_shape, x_init, mask_indices, z_scored_mask, 
                 ...
                 x_init_tcs,
                 ten_ones, max_tt_rank, observed_ratio, epsilon, train_epsilon, backtrack_const, logger, meta, d, subject_folder_name, z_score = 2):
    ...
    def complete(self):
        ...
        optimizer = torch.optim.Adam(self.tensor_factors, lr=lr)
        
        i = 0
        self.grad_norm_val = self.grad_norm_init
        max_iter = 200
        
        self.logger.info("weights = " + str(self.weights))
        ...
        while (i <= max_iter):
            ...
            self.logger.info("weights = " + str(self.weights))
            rec = tl.kruskal_to_tensor((self.weights,self.tensor_factors))
            #kruskal, error_history = parafac(ground_truth_tensor, 81, n_iter_max=1000, init='random', normalize_factors=True, orthogonalise=True, tol=1e-12, random_state=42, verbose=1, return_errors=True, cvg_criterion='abs_rec_error')
            ...
            rse_cost = min(1.0, np.linalg.norm(np.array(rec.data) - self.ground_truth)/np.linalg.norm(self.ground_truth))
            tsc_score = min(1.0, cst.tsc(np.array(rec.data),self.ground_truth, self.ten_ones, self.mask_indices))
            tcs_z_score = min(1.0, tu.tsc_z_score(np.array(rec.data),self.ground_truth,self.ten_ones, self.mask_indices, self.z_scored_mask))
    ...
    def init_variables(self):
        ...
        self.tsc_score_init = min(1.0, cst.tsc(self.x_reconstr_init, self.ground_truth, self.ten_ones, self.mask_indices)).astype('float32')
    ...
    def init_algorithm(self):
        ...
        self.weights = tl.ones(46, requires_grad=False)
