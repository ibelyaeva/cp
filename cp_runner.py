...
import cp_tensor_completion as ct
...
def complete_random_4D():
    ...
    root_dir = config.get('log', 'scratch.cp.dir4D')
    root_dir = config.get('log', 'scratch.cp.dir4Dmulti')
    #subject_meta_folder = fs.create_meta_directory(root_dir, "random")
    
    subject_meta_folder = "/work/scratch/tensor_completion/4D/cp/multisubject/meta_subject2020-08-25_02_00_02/random"
    ...
    for subject_scan_path in subject_list:
        ...
        if accept_subject:
            for item in observed_ratio_list:
                solution_dir, movies_folder, images_folder, results_folder, reports_folder, scans_folder, scans_folder_final, scans_folder_iteration = meta.init_meta_data(root_dir)
                current_runner = ct.CPTensorCompletion(subject_scan_path, item, 4, 1, meta.logger, meta, subject_meta_folder)
